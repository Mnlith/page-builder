...
export class PageBuilder {
  ...
  public remove(name: string) {
    ...
    if (exists) {
      this.virtualFiles = this.virtualFiles.filter((item) =>
        item.name !== name
      );
    } else {
  ...
  public compose(
    ...
    try {
      ...
      const builtHtml = this.build(vFile.name, vFile.content.data);

      // Return the fully merged and built html file as a string
      return builtHtml;
    } catch (e) {
      if (e instanceof Error) {
        throw new Error(
